...
import '../services/client_connections/client.dart';
...
class Booking {
  ...
  Future<void> copyDataToOrder(
    ...
    if (this.customerName == '') this.customerName = UserProvider.user.name;
    this.workerId = worker.phone;
    this.workerName = worker.name;
    this.workerGender = worker.gender;
    this.businessName = SettingsProvider.settings.shopName;
    this.bookingId = this.bookingId == '' ? Uuid().v1() : this.bookingId;
    this.buisnessId = SettingsProvider.appCollection;
    this.userGender = UserProvider.user.gender;
    if (!isWorkerOrder && DeviceProvider.isAllowedNotification) {
      // in case of worker booking for client - dont save the fcm
      this.deviceFCM = await Client().getDeviceFCM();
    }
    if (isWorkerOrder && this.customerPhone != UserProvider.user.phoneNumber) {
    ...
    try {
      this.treatment.price =
          WorkerProvider.worker.treatments[this.treatment.name]!.price;
    } catch (e) {}
